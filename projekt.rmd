...
Rezultati istraživanja provođenih zadnjih nekoliko desetljeća upućuju na to da je gotovo nemoguće dugoročno i konzistentno
...
Premda imamo podatke i o srednjem prošlogodišnjem povratu te povratima za zadnjih 3 i 5 godina, povrati za 10 godina nam najbolje pokazuju koliko je određeni fond uspješan jer nam daju podatke kroz najduži period. Na sljedećem pravokutnom dijagramu prikazani su srednji povrati za 3, 5 i 10 godina. Vidimo da su u prosjeku srednji godišnji povrati za 10 godina najveći.
...
Sljedeći dijagram prikazuje udjele imovine fonda u pojedinim sektorima.
...
labels = c("Basic materials", "Financial services", "Consumer cyclical",
...
boxplot(materials, financial, cyclical, estate, defensive, healthcare, 
        utilities, communication, energy, industrials, technology, names=labels,
...
Kao što je već spomenuto svaki fond ima određeni stil investiranja (Growth, Blend, Value). Zanima nas razlikuju li se uspješnosti fondova s obzirom na stil investiranja koji koriste, odnosno želimo provjeriti imaju li fondovi s određenim stilom investiranja veće povrate nego ostali. Za početak želimo vidjeti ravnaju li se povrati svake od tih kategorija po normalnoj razdiobi kako bi mogli primijeniti analizu varijance.

```{r out.width=c('50%', '50%'), fig.show="hold"}
data_blend <- data[data$investment == c("Blend"),]
hist(data_blend$fund_mean_annual_return_10years, main="Srednji povrat zadnjih 10 godina za Blend")
qqnorm(data_blend$fund_mean_annual_return_10years, main="Srednji povrat zadnjih 10 godina za Blend")
qqline(data_blend$fund_mean_annual_return_10years, col="blue")
```

```{r out.width=c('50%', '50%'), fig.show="hold"}
data_growth <- data[data$investment == c("Growth"),]
hist(data_growth$fund_mean_annual_return_10years, main="Srednji povrat zadnjih 10 godina za Growth")
qqnorm(data_growth$fund_mean_annual_return_10years, main="Srednji povrat zadnjih 10 godina za Growth")
qqline(data_growth$fund_mean_annual_return_10years, col="blue")
```

```{r out.width=c('50%', '50%'), fig.show="hold"}
data_value <- data[data$investment == c("Value"),]
hist(data_value$fund_mean_annual_return_10years,main="Srednji povrat zadnjih 10 godina za Value")
qqnorm(data_value$fund_mean_annual_return_10years, main="Srednji povrat zadnjih 10 godina za Value")
qqline(data_value$fund_mean_annual_return_10years, col="blue")

```

Pretpostavke ANOVA testa su: populacije iz grupa međusobno su nezavisne i normalno distribuirane sa jednakim varijancama. Nezavisnost populacija teško možemo provjeriti stoga ćemo pretpostaviti da su one nezavisne. Iz priloženih grafova možemo zaključiti da nema većih zakrivljenosti u podacima te stoga koristimo testove koji bi koristili kao i kada bi se podaci ravnali po normalnoj distribuciji.
...
Vidimo da se sredine povrata kreću oko sličnih vrijednosti te stoga vrijedi testirati potencijalnu jednakost tih sredina. Pretpostavljamo da su sredine te tri populacije jednake te uz gore navedene pretpostavke provodimo ANOVA test o jednakosti sredina.
...
ANOVA nam samo govori da su sredine tih kategorija međusobno različite, no nas naravno zanima koja od te 3 kategorije ima najveći srednji povrat. Iz pravokutnog dijagrama se može vidjeti da kategorija Growth ima nešto veću sredinu nego ostale dvije kategorije te da Blend ima malo veću sredinu nego Value. Provodimo t-test kako bismo vidjeli ima li kategorija Growth veću sredinu od kategorije Blend. Pretpostavke t-testa su normalnost i nezavisnost podataka i one vrijede zbog početnih pretpostavki ANOVA testa.
...
Vidimo da nam test daje p-vrijednost = 0.0686, što znači da na razini značajnosti 0.05 ne možemo odbaciti pretpostavku da su varijance jednake. Budući da smo dobili takvo rješenje, radimo t-test za dvije populacije koje imaju jednake varijance. Nulta hipoteza t-testa je da su srednji godišnji povrati tih dviju kategorija jednaki, a alternativna hipoteza je da populacija Growth ima veći povrat od populacije Blend.
...
```{r out.width=c('50%', '50%'), fig.show="hold"}
data_blend <- data[data$investment == c("Blend"),]
hist(data_blend$price_earnings, main="P/E ratio(Blend)", xlab="P/E ratio")
...
boxplot(data$price_earnings[data$investment != "<undefined>"] ~ data$investment[data$investment != "<undefined>"],
        ylab= "p/e ratio",
        xlab= "Stil investiranja", col=rainbow(3))
...
boxplot(growthFunds$fund_yield,
        ...
        blendFunds$fund_yield, names=c("Growth","Value","Blend"), col=rainbow(3))
```

Pravokutni dijagram dividendi po stilu investiranja ukazuje na veliku zakrivljenost u podacima kod dividendi growth fondova. Zbog te činjenice ne možemo analizu varijance koja se oslanja na pretpostavku normalnosti svih uzoraka. Za analizu jednakosti sredina koristit ćemo Kruskal-Wallisov test umjesto ANOVA testa. Dodatno testiramo razliku dividendi između value i blend fondova te value i growth fondova. Ako dokažemo da su dividende value fondova veće od ostalih, možemo tvrditi da Value fondovi imaju najveće dividende. Prije samog testiranja razlike provodimo Shapirov test kako bismo utvrdili možemo li za testiranje razlike koristiti t-test ili moramo koristiti neki od neparametarskih testova, u ovom slučaju Wilcoxonov test. Tvrdnja koju testiramo je jesu li dividende value fondova veće od dividendi blend fondova.
...
Pravokutni dijagram za povrate fondova i kategorija mogao bi ukazivati na činjenicu da su povrati fondova i kategorija jednaki s razlikom u većoj varijaciji kod fondova.
...
Za provjere hipoteze o pobjeđivanju fondova analizirat će se zavisnost razlike povrata fonda i povrata kategorije o povratu kateogrije. 
```{r}
...
S grafova se ne može uočiti bilo kakva linearna zavisnost između razlike povrata fonda i kategorije te povrata same kategorije. Analiza reziduala pokazuje nezavisnost reziduala o povratu kategorije.
...
Procijenjeni koeficijenti iznose: b1 = 0.993893 i b0 = -0.157373. Ovom regresijom ne možemo donositi zaključke pobjeđuje li fond svoju kategoriju, već samo donosimo zaključak da fondovi prate svoje kategorije u smislu povrata.
